...
class MultiLayerPerceptron(object):
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
        ...
        dh2 = dz2 * np.where(z2 > 0, 1, 0.01)
        db2 = np.sum(dh2, axis=0)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W2.T

        dh1_from_next_layer = dz1 * np.where(h1 > 0, 1, 0)
        dh1 = dh1_from_next_layer + dh3
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        for it in range(epoch):
            ...
            # Gradient update
            self.model['W1'] -= grad['dW1'] * learning_rate
            self.model['b1'] -= grad['db1'] * learning_rate
            self.model['W2'] -= grad['dW2'] * learning_rate
            self.model['b2'] -= grad['db2'] * learning_rate
            self.model['W3'] -= grad['dW3'] * learning_rate
            self.model['b3'] -= grad['db3'] * learning_rate
            self.model['W4'] -= grad['dW4'] * learning_rate
            self.model['b4'] -= grad['db4'] * learning_rate
